...
const Header: React.FunctionComponent<HeaderProps> = ({ logOut, user }) => {
    ...
    const adminPanel = [
        ...
        { text: 'Profil', icon: <PersonOutlineOutlinedIcon sx={menuIconsStyles} />, path: '/profile', divider: true },
        { text: 'Çıkış Yap', icon: <LogoutIcon sx={menuIconsStyles} />, onClick: logOut },
    ]
                ...
                <PopButton
                    button={
                        <IconButton sx={{ padding: '0' }}>
                            <Icon icon={<ArrowDropDownIcon width={17} height={9} color={colors.action.actionActiveLight} />} />
                        </IconButton>
                    }
                    popArea={
                        <List
